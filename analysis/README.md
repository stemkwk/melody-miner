...
- **스크립트**: `scripts/analysis/visualize_piano_roll.py`
- **기본 실행 (최신 결과물 자동 분석)**
  ```bash
  python scripts/analysis/visualize_piano_roll.py
  ```
...
- **특정 파일 직접 분석**
  ...
  ```bash
  python scripts/analysis/visualize_piano_roll.py --midi "output/특정폴더/03_accompaniment_full.mid"
  ```
...
- **스크립트**: `scripts/analysis/diagnose_vocoder_wobble.py`
- **기본 실행 (기본 레퍼런스 분석)**
  ```bash
...
- **특정 파일 직접 분석**
  ...
  ```bash
  python scripts/analysis/diagnose_vocoder_wobble.py --source "경로/다른_입력보컬.wav" --reference "경로/다른_목표화자.wav"
  ```
