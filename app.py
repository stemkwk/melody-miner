"""melody-miner integrated Gradio demo.

Upload an input WAV (+ optional target-speaker reference) → accompaniment
generation + voice conversion → play vocal / accompaniment / final mix at once.

Checkpoints are **auto-fixed**: the ckpt under ``checkpoints/m2a/`` is used as
M2A and the ckpt under ``checkpoints/tnp/`` as TNP, loaded at startup.
(You can also pass ``--m2a-checkpoint`` / ``--tnp-checkpoint`` explicitly.)

Run:
    python app.py                 # auto-use checkpoints/m2a, checkpoints/tnp
    python app.py --share
"""
...
def main() -> None:
    ...
    # TNP: explicit arg, else auto-fixed from checkpoints/tnp/ (None → skip VC)
