...
def apply_vocal_postprocessing(vocal: np.ndarray, sr: int,
    ...
    band-aid for the predicted-mel dullness ("watery"): the extra harmonics add
...
def postprocess_vocal_file(in_path: Path, out_path: Path,
    ...
    # [FIX] Prevent hard clipping and volume-balance collapse:
    # If a HighShelf EQ pushes peaks above 1.0 and we only apply np.clip, the
    # clipped peaks inflate the average level (RMS), breaking the mix balance
