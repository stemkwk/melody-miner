...
Output layout under ``out_dir``:
    01_input.wav           input copy
    02_melody.mid          transcribed melody MIDI
    03_accompaniment.mid   generated accompaniment MIDI
    03_accompaniment.wav   generated accompaniment WAV (when a soundfont exists)
    04_vocal.wav           converted vocal (or input fallback when VC is skipped)
    05_final_mix.wav       ← final result (vocal + accompaniment)
"""
...
def run_full(
    ...
    mode="full"  Branch A (accompaniment) + Branch B (voice conversion) → mix(converted vocal + accompaniment)
    mode="m2a"   Branch A only - accompaniment generation → mix(original input + accompaniment). No VC
    mode="tnp"   Branch B only - converted vocal only. No accompaniment/mix
    ...
    if accomp_wav is not None:
        if voc is not None:
            vocal_stem = vocal_post or voc["vocal_wav"]   # full: prefer post-processed take
        elif not is_midi_input:
            vocal_stem = input_copy                    # m2a + audio input: original WAV
        else:
            vocal_stem = acc["melody_wav"]             # m2a + MIDI input: rendered melody
