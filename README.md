...
## 설치 (Windows + Python 3.12 + NVIDIA CUDA — 검증됨)
```bash
bash scripts/setup_env_windows.sh        # pip만, conda/Rust 불필요
```
스크립트가 하는 일(수동으로도 동일):
```bash
...
# + soundfonts/GeneralUser.sf2 (GM 사운드폰트, 없으면 fallback 다운로드)
```
> **Linux/conda 대안**: `conda env create -f environment.yml` (rust/ffmpeg 포함 →
> deepfilternet까지 설치 가능). 자세한 핀·근거는 `requirements.txt` 상단 주석 참고.
